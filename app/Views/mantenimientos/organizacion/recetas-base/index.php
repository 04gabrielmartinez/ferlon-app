...
$productosRecetaBase = is_array($productosRecetaBase ?? null) ? $productosRecetaBase : [];
...
foreach ($productosRecetaBase as $p) {
    if ((int) ($p['receta_base_id'] ?? 0) > 0) {
        $productosConReceta++;
    }
}
$productosSinReceta = max(0, count($productosRecetaBase) - $productosConReceta);
?>
<?php
$codReceta = ((int) ($receta['id'] ?? 0) > 0)
    ? ('RB-' . str_pad((string) ((int) ($receta['id'] ?? 0)), 6, '0', STR_PAD_LEFT))
    : '';
$productoSeleccionadoId = (int) ($receta['producto_articulo_id'] ?? 0);
$imprimirUrl = $productoSeleccionadoId > 0
    ? ('/mantenimientos/organizacion/recetas-base/imprimir?producto_id=' . $productoSeleccionadoId)
    : '';
?>
<div class="container-fluid px-0">
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-header bg-white border-0 pt-3 px-3 px-md-4">
            <h2 class="h6 mb-0 text-secondary">Mantenimientos / Organizacion / Receta Base</h2>
            <small class="text-muted">Configuracion de productos base y sus insumos de receta</small>
        </div>
        <div class="card-body px-3 px-md-4 pb-4">
            <div class="section-card mb-3">
                <div class="section-title">Formulario de Receta Base</div>
                <form method="post" action="/mantenimientos/organizacion/recetas-base" id="recetaBaseForm" class="employee-form">
                    <input type="hidden" name="_csrf" value="<?= htmlspecialchars((string) ($csrf ?? '')) ?>">
                    <input type="hidden" name="id" value="<?= (int) ($receta['id'] ?? 0) ?>">
                    <input type="hidden" name="editar_rendimiento" id="editar_rendimiento" value="0">

                    <div class="row g-3">
                        <div class="col-12 col-md-4">
                            <label class="form-label small mb-1">Cod receta</label>
                            <div class="input-group input-group-sm">
                                <input type="text" class="form-control" value="<?= htmlspecialchars($codReceta) ?>" readonly placeholder="Se genera al guardar">
                                <button type="button" class="btn btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#productoRecetaPickerModal" aria-label="Buscar producto receta base">
                        ...
                        <div class="col-12 col-md-8">
                            <label class="form-label small mb-1">Producto (Receta base)</label>
                            <input type="hidden" name="producto_articulo_id" id="producto_articulo_id" value="<?= (int) ($receta['producto_articulo_id'] ?? 0) ?>">
                            <input type="text" id="producto_receta_label" class="form-control form-control-sm" value="<?= htmlspecialchars(trim((string) ($productoSeleccionado['codigo'] ?? '') . ' - ' . (string) ($productoSeleccionado['descripcion'] ?? ''))) ?>" readonly placeholder="Seleccione un producto usando la lupa de Cod receta">
                        </div>

                        <div class="col-12 col-md-3">
                            <label class="form-label small mb-1">Rendimiento</label>
                            <input type="number" step="0.0001" min="0.0001" name="rendimiento" id="receta_rendimiento" class="form-control form-control-sm" value="<?= htmlspecialchars((string) ($receta['rendimiento'] ?? '1')) ?>" readonly>
                        </div>

                        <div class="col-12 col-md-3">
                            <label class="form-label small mb-1">Unidad</label>
                            <?php $unidadR = strtolower((string) ($receta['unidad_rendimiento'] ?? 'kg')); ?>
                            <select name="unidad_rendimiento" id="receta_unidad_rendimiento" class="form-select form-select-sm" disabled>
                                <option value="g" <?= $unidadR === 'g' ? 'selected' : '' ?>>g</option>
                                <option value="kg" <?= $unidadR === 'kg' ? 'selected' : '' ?>>kg</option>
                                <option value="lb" <?= $unidadR === 'lb' ? 'selected' : '' ?>>lb</option>
                                <option value="oz" <?= $unidadR === 'oz' ? 'selected' : '' ?>>oz</option>
                                <option value="u" <?= $unidadR === 'u' ? 'selected' : '' ?>>u</option>
                            </select>
                        </div>

                        <div class="col-12 col-md-3 d-flex align-items-end">
                            <button type="button" class="btn btn-outline-secondary btn-sm rounded-pill px-3" id="btnEditarRendimiento">Editar rendimiento</button>
                        </div>
                    </div>
                    ...
                    <div id="detalle_lock_hint" class="alert alert-info py-2 px-3 small mb-3">
                        Selecciona un producto para habilitar el detalle de insumos.
                    </div>
                    ...
                    <div class="d-flex gap-2 mt-3">
                        ...
                        <a href="/mantenimientos/organizacion/recetas-base" class="btn btn-outline-secondary btn-sm rounded-pill px-3">Cancelar</a>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="modal-header">
                <div>
                    <h5 class="modal-title d-flex align-items-center gap-2" id="productoRecetaPickerModalLabel">
                        ...
                        <span>Seleccionar producto receta base</span>
                    </h5>
                    <small class="text-muted">Solo se muestran articulos con "Receta base" activo</small>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
            </div>
            <div class="modal-body">
                <div class="d-flex flex-wrap gap-2 mb-3">
                    <span class="badge text-bg-success">Creadas: <?= $productosConReceta ?></span>
                    <span class="badge text-bg-secondary">Pendientes: <?= $productosSinReceta ?></span>
                </div>
                <table id="productoRecetaPickerTable" class="table table-hover align-middle w-100 employee-picker-table">
                    ...
                    <tbody>
                        <?php foreach ($productosRecetaBase as $p): $pid = (int) ($p['id'] ?? 0); if ($pid <= 0) continue; ?>
                            <?php $creada = (int) ($p['receta_base_id'] ?? 0) > 0; ?>
                            <tr class="js-producto-receta-row" data-producto-id="<?= $pid ?>" data-receta-id="<?= (int) ($p['receta_base_id'] ?? 0) ?>">
                                <td><?= htmlspecialchars((string) ($p['codigo'] ?? '')) ?></td>
                                <td><?= htmlspecialchars((string) ($p['descripcion'] ?? '')) ?></td>
                                <td>
                                    <span class="badge <?= $creada ? 'text-bg-success' : 'text-bg-secondary' ?>">
        ...
        <div class="modal-content border-0 shadow">
            <div class="modal-header">
                <div>
                    ...
                    <small class="text-muted">Solo se muestran articulos con "Insumo receta" activo</small>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
            </div>
            <div class="modal-body">
...
<script>
(() => {
    const rendimientoInput = document.getElementById('receta_rendimiento');
    const unidadSel = document.getElementById('receta_unidad_rendimiento');
    const editarRend = document.getElementById('editar_rendimiento');
    const btnEditarRend = document.getElementById('btnEditarRendimiento');
    ...
    const syncDetalleLockState = () => {
        const hasProducto = Number(productoIdHidden?.value || '0') > 0;
        if (detalleFieldset) detalleFieldset.disabled = !hasProducto;
        if (detalleLockHint) {
            if (hasProducto) detalleLockHint.classList.add('d-none');
            else detalleLockHint.classList.remove('d-none');
        }
    };
    ...
    const showErrorToast = (message) => showToast(message, 'danger', 'Receta Base');

    if (btnEditarRend && rendimientoInput && unidadSel && editarRend) {
        btnEditarRend.addEventListener('click', () => {
            const editable = rendimientoInput.hasAttribute('readonly');
            if (editable) {
                rendimientoInput.removeAttribute('readonly');
                unidadSel.disabled = false;
                editarRend.value = '1';
                btnEditarRend.textContent = 'Bloquear rendimiento';
                return;
            }
            rendimientoInput.setAttribute('readonly', 'readonly');
            unidadSel.disabled = true;
            editarRend.value = '0';
            if (!rendimientoInput.value || Number(rendimientoInput.value) <= 0) {
                rendimientoInput.value = '1';
            }
            unidadSel.value = 'kg';
            btnEditarRend.textContent = 'Editar rendimiento';
        });
    }
    ...
    document.addEventListener('click', (event) => {
        ...
        if (productoRow) {
            const productoId = Number(productoRow.getAttribute('data-producto-id') || '0');
            const recetaId = Number(productoRow.getAttribute('data-receta-id') || '0');
            if (productoId > 0) {
                if (recetaId > 0) {
                    sessionStorage.setItem('rb_notice', JSON.stringify({
                        type: 'info',
                        title: 'Receta Base',
                        message: 'Editando datos de receta.',
                    }));
                } else {
                    sessionStorage.setItem('rb_notice', JSON.stringify({
                        type: 'success',
                        title: 'Receta Base',
                        message: 'Creando receta nueva para el producto seleccionado.',
                    }));
                }
                window.location.href = '/mantenimientos/organizacion/recetas-base?producto_id=' + encodeURIComponent(String(productoId));
            }
            return;
        }
    ...
    const pendingNotice = sessionStorage.getItem('rb_notice');
    if (pendingNotice) {
        try {
            const data = JSON.parse(pendingNotice);
            showToast(String(data.message || ''), String(data.type || 'info'), String(data.title || 'Receta Base'));
        } catch (_err) {}
        sessionStorage.removeItem('rb_notice');
    }
