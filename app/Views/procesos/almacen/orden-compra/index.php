...
$codigoCompra = (string) ($codigoCompra ?? '');
...
$proveedorSelId = (int) ($ocSeleccionada['proveedor_id'] ?? 0);
$ocIdSel = (int) ($ocSeleccionada['id'] ?? 0);
$proveedorSelLabel = (string) ($ocSeleccionada['proveedor_label'] ?? '');
$proveedorSelRnc = (string) ($ocSeleccionada['proveedor_rnc'] ?? '');
$condicionPagoSel = (string) ($ocSeleccionada['condicion_pago'] ?? '');
$comentarioSel = (string) ($ocSeleccionada['comentario'] ?? '');
$subtotalSel = (string) ($ocSeleccionada['subtotal'] ?? '0');
$descSel = (string) ($ocSeleccionada['total_descuento'] ?? '0');
$descPctSel = (string) ($ocSeleccionada['descuento_general_pct'] ?? '0');
$impuestoSel = (string) ($ocSeleccionada['impuesto'] ?? '0');
$totalSel = (string) ($ocSeleccionada['total_compra'] ?? '0');
$monedaSel = (string) ($ocSeleccionada['moneda'] ?? 'DOP');
$detallesSel = is_array($ocSeleccionada['detalles'] ?? null) ? $ocSeleccionada['detalles'] : [];
...
$codigoCompraDisplay = $ocIdSel > 0 ? $codigoCompra : 'Se asigna al guardar';
?>
<div class="container-fluid px-0">
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-header bg-white border-0 pt-3 px-3 px-md-4">
            <h2 class="h6 mb-0 text-secondary">Procesos / Almacen / Orden de compra</h2>
            <small class="text-muted">Creacion de OC para proveedor y base para entradas parciales</small>
        </div>
        <div class="card-body px-3 px-md-4 pb-4">
            <div class="section-card mb-3">
                <div class="section-title">Formulario de Orden de Compra</div>
                <form method="post" action="/procesos/almacen/orden-compra" id="ordenCompraForm" class="employee-form">
                    <input type="hidden" name="_csrf" value="<?= htmlspecialchars((string) ($csrf ?? '')) ?>">
                    <input type="hidden" name="oc_id" value="<?= $ocIdSel ?>">
                    <input type="hidden" name="empleado_id" id="empleado_id_oc" value="<?= $empleadoId ?>">
                    ...
                    <div class="row g-3">
                        <div class="col-12 col-lg-4">
                            <div class="row g-3">
                                <div class="col-12">
                                    <label class="form-label small mb-1">Codigo compra</label>
                                    <div class="input-group input-group-sm">
                                        <?php if ($ocIdSel > 0): ?>
                                            <input type="text" class="form-control" name="codigo_compra" value="<?= htmlspecialchars($codigoCompra) ?>" readonly>
                                        <?php else: ?>
                                            <input type="text" class="form-control" value="<?= htmlspecialchars($codigoCompraDisplay) ?>" readonly>
                                        <?php endif; ?>
                                ...
                                <div class="col-12">
                                    ...
                                    <?php if ($puedeCambiarEmpleado): ?>
                                        <div class="input-group input-group-sm">
                                            <input type="text" id="empleado_nombre_oc" class="form-control" value="<?= htmlspecialchars($empleadoNombre) ?>" readonly>
                                            <button
                                                type="button"
                                                class="btn btn-outline-secondary js-open-employee-picker"
                                                data-employee-target="#empleado_id_oc"
                                                aria-label="Buscar empleado"
                                            >
                                                <i class="bi bi-search"></i>
                                            </button>
                                        </div>
                                        <small class="text-muted">ID: <span id="empleado_id_badge"><?= $empleadoId ?></span></small>
                                    <?php else: ?>
                                        <input type="text" id="empleado_nombre_oc" class="form-control form-control-sm" value="<?= htmlspecialchars($empleadoNombre) ?>" readonly>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-12 col-lg-4">
                            <div class="row g-3">
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="comentario" class="form-control form-control-sm" rows="2" placeholder="Observaciones de la orden de compra"><?= htmlspecialchars($comentarioSel) ?></textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-12 col-lg-4">
                            <div class="border rounded-4 p-3 bg-light-subtle h-100">
                                ...
                                <div class="row g-2">
                                    ...
                                    <div class="col-12">
                                        <label class="form-label small mb-1">Total compra</label>
                    ...
                    <div class="d-flex flex-wrap align-items-center gap-2 mb-2">
                        <div class="section-title mb-0">Detalle de Articulos a Comprar</div>
                    </div>
                    ...
                    <div class="d-flex gap-2 mt-3">
                        <button type="submit" class="btn btn-primary btn-sm rounded-pill px-3" <?= $lockBloqueado ? 'disabled' : '' ?>>Guardar OC</button>
                        <?php if ($ocIdSel > 0): ?>
                            <a class="btn btn-outline-secondary btn-sm rounded-pill px-3" target="_blank" href="/procesos/almacen/orden-compra/imprimir?id=<?= $ocIdSel ?>">Imprimir</a>
                        <?php endif; ?>
                        <a href="/procesos/almacen/orden-compra" class="btn btn-outline-secondary btn-sm rounded-pill px-3">Cancelar</a>
                    </div>
                </form>
            </div>
        </div>
                ...
                <table id="articuloCompraPickerTable" class="table table-hover align-middle w-100 employee-picker-table">
                    ...
                    <tbody>
                        <?php foreach ($articulos as $a): $aid = (int) ($a['id'] ?? 0); if ($aid <= 0) continue; ?>
                            <?php
                                $unidadBaseRow = strtolower((string) ($a['unidad_base_id'] ?? 'u'));
                                $stockNum = $unidadBaseRow === 'u'
                                    ? (float) ($a['stock_actual'] ?? 0)
                                    : (float) ($a['stock_actual_kg'] ?? 0);
...
<script>
(() => {
    const formEl = document.getElementById('ordenCompraForm');
    const empleadoIdEl = document.getElementById('empleado_id_oc');
    const empleadoNombreEl = document.getElementById('empleado_nombre_oc');
    ...
    const showToast = (message, type = 'danger', title = 'Orden de compra') => {
    ...
    const renderDetalle = () => {
        ...
        detalleItems.forEach((it) => {
            ...
            tr.innerHTML = `
                <td>${codigo}<input type="hidden" name="detalle_codigo[]" value="${codigo}"><input type="hidden" name="detalle_articulo_id[]" value="${aid}"></td>
    ...
    const upsertDetalle = () => {
        ...
        if (unidad !== 'u' && !isPesoUnidad(unidad) && pesoPorUnidad <= 0) {
            return showToast('Peso x unidad es obligatorio cuando la unidad no es "u".', 'warning');
        }
        if (unidad !== 'u' && isPesoUnidad(unidad) && pesoPorUnidad <= 0) {
    ...
    document.addEventListener('click', (event) => {
        ...
        if (ocRow) {
            ...
            if (id > 0) window.location.href = '/procesos/almacen/orden-compra?id=' + encodeURIComponent(String(id));
            return;
        }
                ...
                columns: [
                    {
                        ...
                        render: (data, _type, row) => {
                            ...
                            return `${codigo}<input type="hidden" name="detalle_codigo[]" value="${codigo}"><input type="hidden" name="detalle_articulo_id[]" value="${aid}">`;
                        },
                    },
                    {
                        data: 'descripcion',
